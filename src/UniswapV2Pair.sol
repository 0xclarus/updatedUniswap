...
import {IUniswapV2Callee} from "../contracts/interfaces/IUniswapV2Callee.sol";
...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function _safeTransfer(address token, address to, uint256 value) private {
        ...
        if (!(success && (data.length == 0 || abi.decode(data, (bool))))) {
            revert UniswapV2__TRANSFER_FAILED();
        }
    }
    ...
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) private {
        ...
        if (balance0 <= uint112(2 ** 112 - 1) && balance1 <= uint112(2 ** 112 - 1)) {
            revert UniswapV2__OVERFLOW();
        }
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (liquidity > 0) {
            ...
            revert UniswapV2__INSUFFICIENT_LIQUIDITY_MINTED();
        }
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        ...
        if (amount0 > 0 && amount1 > 0) {
            ...
            revert UniswapV2__INSUFFICIENT_LIQUIDITY_BURNED();
        }
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
        if (amount0Out > 0 || amount1Out > 0) {
            revert UniswapV2__INSUFFICIENT_OUTPUT_AMOUNT();
        }

        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        if (amount0Out < _reserve0 && amount1Out < _reserve1) {
            revert UniswapV2__INSUFFICIENT_LIQUIDITY();
        }
        ...
        if (amount0In > 0 || amount1In > 0) {
            revert UniswapV2__INSUFFICIENT_INPUT_AMOUNT();
        }
        {
